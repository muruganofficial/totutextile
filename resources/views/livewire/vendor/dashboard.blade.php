<div class="space-y-10 font-inter">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Welcome back, {{ $vendor->company_name ?? 'Atelier Partner' }}</h1>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-8">
        ...
        <div class="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-150 dark:border-gray-800 shadow-sm flex flex-col justify-between h-36">
            <div class="flex items-center justify-between">
                <span class="text-2xl font-bold text-gray-900 dark:text-white">₹{{ number_format($grossEarnings, 2) }}</span>
        ...
        <div class="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-150 dark:border-gray-800 shadow-sm flex flex-col justify-between h-36">
            <div class="flex items-center justify-between">
                <span class="text-2xl font-bold text-gray-900 dark:text-white">₹{{ number_format($netEarnings, 2) }}</span>
                <span class="flex items-center gap-1 text-[10px] font-black text-emerald-500 uppercase">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 15l7-7 7 7"></path></svg>
                    12.09%
                </span>
        ...
        <div class="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-150 dark:border-gray-800 shadow-sm flex flex-col justify-between h-36">
            <div class="flex items-center justify-between">
                <span class="text-2xl font-bold text-gray-900 dark:text-white">₹{{ number_format($commissionDeducted, 2) }}</span>
            ...
            <div class="flex items-center gap-2.5 text-xs text-gray-400 font-semibold mt-4">
                <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                Commissions
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-150 dark:border-gray-800 shadow-sm flex flex-col justify-between h-36">
            <div class="flex items-center justify-between">
                <span class="text-2xl font-bold text-gray-900 dark:text-white">{{ count($products) }}</span>
                <span class="flex items-center gap-1 text-[10px] font-black text-emerald-500 uppercase">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 15l7-7 7 7"></path></svg>
                    2.00%
                </span>
    ...
    <!-- Income Report Custom Chart Widget -->
    <div class="bg-white dark:bg-gray-900 p-8 rounded-2xl border border-gray-150 dark:border-gray-800 shadow-sm space-y-6">
        <div class="flex items-center justify-between border-b border-gray-100 dark:border-gray-800 pb-4">
            <h3 class="font-bold text-sm uppercase tracking-wider text-gray-900 dark:text-white">Monthly Earnings - {{ date('Y') }}</h3>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            <h2 class="text-lg font-bold text-gray-900 dark:text-white">Earnings & Settlements Log</h2>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-sm border border-gray-150 dark:border-gray-800">
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-850">
                                @foreach($earnings as $earn)
                                    <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition">
                                        <td class="px-6 py-4 font-bold">{{ $earn->order->order_number ?? 'N/A' }}</td>
                                        <td class="px-6 py-4">₹{{ number_format($earn->gross_amount, 2) }}</td>
                                        <td class="px-6 py-4 text-rose-500 font-semibold">-₹{{ number_format($earn->commission_deducted, 2) }}</td>
                                        <td class="px-6 py-4 font-bold text-emerald-500">₹{{ number_format($earn->net_earnings, 2) }}</td>
                                        <td class="px-6 py-4 text-right">
                                            <span class="px-2.5 py-1.5 text-[9px] font-bold rounded-md uppercase tracking-wider 
                                                {{ $earn->status === 'Pending' ? 'bg-amber-500/10 text-amber-500' : '' }}
                                                {{ $earn->status === 'Paid' ? 'bg-emerald-500/10 text-emerald-500' : '' }}
                                            ">
                                                {{ $earn->status }}
                                            </span>
                                        </td>
                ...
                @else
                    <div class="p-10 text-center text-gray-55 border border-gray-150">
                        No settlements or earnings logged yet.
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="lg:col-span-1 space-y-6">
            <h2 class="text-lg font-bold text-gray-900 dark:text-white">Active Products</h2>
            
            <div class="space-y-4 max-h-[500px] overflow-y-auto pr-2">
                @forelse($products as $prod)
                    <div class="bg-white dark:bg-gray-900 p-5 rounded-2xl shadow-sm border border-gray-150 dark:border-gray-800 flex items-center justify-between gap-4">
                        <div>
                            <h4 class="text-sm font-bold text-gray-955 dark:text-white line-clamp-1">{{ $prod->name }}</h4>
                            <p class="text-xs text-gray-400 mt-0.5">SKU: {{ $prod->sku }}</p>
                            <p class="text-sm font-black text-luxury-gold mt-2">₹{{ number_format($prod->selling_price, 2) }}</p>
                        </div>
                        <div class="text-right flex-shrink-0">
                            <span class="px-2 py-1 text-[9px] font-bold uppercase tracking-wider rounded-md bg-emerald-500/10 text-emerald-500">
                                {{ $prod->total_stock }} In Stock
                            </span>
                        </div>
                    </div>
                @empty
                    <div class="bg-white dark:bg-gray-900 p-10 rounded-2xl text-center border border-gray-150 shadow-sm">
                        <p class="text-sm text-gray-500">No products uploaded yet.</p>
                    </div>
                @endforelse
            </div>
        </div>
